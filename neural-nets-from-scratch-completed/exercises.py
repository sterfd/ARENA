...
class Neuron:
    ...
    def compute_output(self, inputs: list[float]) -> float:
        ...
        # TODO: Fill this in!
        wx = [w * x for w, x in zip(self.weights, inputs)]
        return relu(sum(wx) + self.bias)

        # ***~~~actual is a negative number, and expect is 0, so we're missing a ReLu
...
def forward_pass_single_layer(input: list[float], layer: list[Neuron]) -> list[float]:
    ...
    for neuron in layer:
        ...
        # TODO: Fill this in!
        outputs.append(neuron.compute_output(input))
    return outputs
    raise NotImplementedError()
...
def x_squared_plus_y_squared_plus_5_thousand_times() -> tuple[np.ndarray, np.ndarray]:
    ...
    x2y2 = sum(points[:, 0] ** 2 + points[:, 1] ** 2)
    print(
        x2y2.shape
    )  # before taking sum, had error: grad can be implicitly created only for scalar outputs - you call backward on the loss -> not the ouptuts - this needs to be 1 value, not a vector or a tensor
    x2y2.backward()
    xy_values = points.detach().numpy()
    xy_grad = points.grad
    return (xy_values, xy_grad)
...
def apply_linear_function_to_input(
    ...
    return einops.einsum(
        linear_function, input_to_function, "output input, batch input -> batch output"
    )
...
example_output = forward(x=t.ones((10, 784)), neural_net=new_neural_net)
...
def loss_function(
    ...
    # TODO: Implement this
    loss = ((expected_outputs - actual_outputs) ** 2).mean()
    return loss
...
def tune_weights_once(
    ...
    output = forward(inputs, neural_net)
...
class SimpleNeuralNet(t.nn.Module):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(
            *args, **kwargs
        )  # telling this class is a child of t.nn.Module and is bringing in all its properties
